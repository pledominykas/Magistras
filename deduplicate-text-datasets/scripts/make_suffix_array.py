...
for jobstart in range(0, total_jobs, jobs_at_once):
    ...
    for i in range(jobstart,jobstart+jobs_at_once):
        ...
        cmd = "\"./target/debug/dedup_dataset.exe\" make-part --data-file %s --start-byte %d --end-byte %d"%(sys.argv[1], s, e)
        started.append((s, e))
        print(cmd)
...
print("\"./target/debug/dedup_dataset.exe\" merge --output-file %s --suffix-path %s --num-threads %d"%("tmp/out.table.bin", torun, mp.cpu_count()))
pipe = os.popen("\"./target/debug/dedup_dataset.exe\" merge --output-file %s --suffix-path %s --num-threads %d"%("tmp/out.table.bin", torun, mp.cpu_count()))
output = pipe.read()
